    ...
    return (
        <>
            ...
            <div className="wrap">
                {catData.length > 0 &&
                    catData
                        .filter((cat) => !showLiked || liked[cat.id])
                        .map((cat, index) =>
                            <div className="card" key={cat.id}>
                                ...
                                </div><button className="card__btn_like" onClick={() => handleLikeClick(cat.id)}>
                                    {liked[cat.id] ? (<svg width="64" height="60" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
